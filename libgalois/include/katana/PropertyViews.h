...
KATANA_EXPORT Result<std::vector<arrow::Array*>> ExtractArrays(
    const PropertyGraph::PropertyView& pview,
...
MakeNodePropertyViews(
  ...
  return MakePropertyViews<PropTuple>(pg->node_property_view(), properties);
}
...
MakeEdgePropertyViews(
    const PropertyGraph* pg, const std::vector<std::string>& properties) {
  return MakePropertyViews<PropTuple>(pg->edge_property_view(), properties);
}
